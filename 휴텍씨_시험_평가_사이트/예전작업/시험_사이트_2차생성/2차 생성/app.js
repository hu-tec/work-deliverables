...
function renderExaminee() {
  const rows = data.exams.map((exam) => [exam.id, exam.title, exam.round, exam.grade, exam.type, exam.date, `${exam.fee}원`, statusBadge(exam.status), `<button class="btn primary" data-open-dialog="apply">신청</button>`]);
  const resultRows = data.exams.map((exam) => [exam.title, exam.type, exam.date, statusBadge(exam.result), exam.result === "합격" ? `<button class="btn success" data-open-dialog="certificate">자격증 신청</button>` : `<button class="btn ghost" disabled>대기</button>`]);
  const paymentRows = data.payments.slice(0, 3).map((payment) => [payment[0], payment[1], payment[2], payment[3], statusBadge(payment[4]), `<button class="btn ghost" data-open-dialog="receipt">영수증</button>`]);
  return html`
    <section class="section">
      <div class="workflow">
        ${["시험 접수", "결제", "시험 응시", "결과 확인", "자격증 신청"].map((step, index) => `<div class="workflow-step"><span>STEP ${index + 1}</span><strong>${step}</strong><p>수험생 핵심 시나리오 연결</p></div>`).join("")}
      </div>
    </section>
    <section class="section">
      <div class="section-head"><div><h2>시험 접수 목록</h2><p>상세/신청, 변경, 취소, 환불 가능 여부를 확인합니다.</p></div><div class="btn-row"><button class="btn ghost" data-open-dialog="change">접수 정보 변경</button><button class="btn danger" data-open-dialog="refund">취소/환불 확인</button></div></div>
      ${table(["ID", "시험명", "회차", "등급", "유형", "시험일", "응시료", "상태", "액션"], rows)}
    </section>
    <section class="section exam-screen">
      <div class="panel">
        <h3>시험 시작 전 안내</h3>
        <p class="muted">전체화면 상태, 남은 시간, 자동저장, 제출 확인을 제공합니다.</p>
        <div class="timer" id="examTimer">39:58</div>
        <div class="question-nav" style="margin-top:12px">${[1,2,3,4,5,6,7,8].map((n) => `<button class="${n === 1 ? "active" : ""}">${n}</button>`).join("")}</div>
      </div>
      <div class="panel">
        <h3>시험 진행 화면</h3>
        ...
        <label class="field"><span>답안 에디터</span><textarea id="answerEditor">자동저장 대기 중인 답안입니다.</textarea></label>
        <div class="btn-row" style="margin-top:12px"><button class="btn ghost" id="autosaveButton">임시저장</button><button class="btn primary" data-open-dialog="submit">제출</button><button class="btn ghost" data-open-dialog="next">다음 차시</button></div>
      </div>
      <div class="panel">
        <h3>결과/자격증</h3>
        ${table(["시험명", "유형", "응시일", "결과", "액션"], resultRows)}
      </div>
    </section>
    <section class="section grid two">
      <div class="panel">
        <div class="section-head"><div><h2>결제 내역</h2><p>접수 후 결제, 포인트 결제, 환불 상태를 확인합니다.</p></div><button class="btn primary" data-open-dialog="payment">결제하기</button></div>
        ${table(["결제ID", "내용", "수단", "금액", "상태", "액션"], paymentRows)}
      </div>
      <div class="panel">
        <div class="section-head"><div><h2>자격증 신청</h2><p>합격 결과를 선택해 배송 정보를 등록합니다.</p></div></div>
        <div class="form-grid">
          <label class="field"><span>신청 시험</span><select><option>프롬프트 교육 3급</option><option>웹툰 번역 일반 1급</option></select></label>
          <label class="field"><span>수령 방식</span><select><option>온라인 발급</option><option>우편 수령</option></select></label>
          <label class="field full"><span>주소/이메일</span><input value="seoul@example.test"></label>
        </div>
        <div class="btn-row" style="margin-top:12px"><button class="btn success" data-open-dialog="certificateApply">자격증 신청</button><button class="btn ghost" data-open-dialog="certificateHistory">신청 내역</button></div>
      </div>
    </section>
  `;
}
