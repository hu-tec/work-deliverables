...
export const routes = [
  ...
  { path: "/admin/members/[id]", title: "회원 상세", roles: ["admin"], designSource: "../../figma_pdf/2024/회원관리_상세_전체.pdf" },
...
export function canAccess(role, path) {
  const route = routes.find((item) => routeMatches(item.path, path));
  return Boolean(route && route.roles.includes(role));
}

export function routeMatches(routePath, path) {
  if (routePath === path) {
    return true;
  }
  ...
  return routeParts.every((part, index) => part.startsWith("[") && part.endsWith("]") ? Boolean(pathParts[index]) : part === pathParts[index]);
}
