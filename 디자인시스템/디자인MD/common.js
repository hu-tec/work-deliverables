...
const docButtons = document.querySelectorAll("[data-doc-src]");
const docFrame = document.querySelector("[data-doc-frame]");
const docRender = document.querySelector("[data-doc-render]");
const docTitle = document.querySelector("[data-doc-current-title]");
const docOpen = document.querySelector("[data-doc-open]");

async function loadMarkdownDoc(src, title) {
...
docButtons.forEach((button) => {
    ...
    loadMarkdownDoc(src, title);
    });
});

const initialDoc = document.querySelector("[data-doc-src].active") || docButtons[0];
if (initialDoc) {
    loadMarkdownDoc(initialDoc.dataset.docSrc, initialDoc.dataset.docTitle || initialDoc.textContent.trim());
}
